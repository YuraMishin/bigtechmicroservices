...
func (s *ServiceSuite) TestGetPart_NotFound() {
	...
	ctx := context.Background()
	s.partRepository.EXPECT().
		GetPart(ctx, "non-existent-uuid").
		Return(model.Part{}, model.ErrPartNotFound)

	// Act
	result, err := s.service.GetPart(ctx, "non-existent-uuid")
...
func (s *ServiceSuite) TestGetPart_RepositoryError() {
	...
	s.partRepository.EXPECT().
		GetPart(ctx, "test-uuid").
		Return(model.Part{}, expectedError)

	// Act
	result, err := s.service.GetPart(ctx, "test-uuid")
...
func (s *ServiceSuite) TestGetPart_EmptyUUID() {
	// Arrange
	ctx := context.Background()
	s.partRepository.EXPECT().
		GetPart(ctx, "").
