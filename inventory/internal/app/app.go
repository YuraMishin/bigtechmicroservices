...
import (
	...
	"github.com/YuraMishin/bigtechmicroservices/platform/pkg/grpc/health"
...
func (a *App) initListener(_ context.Context) error {
	listener, err := net.Listen("tcp", fmt.Sprintf(":%d", config.AppConfig().InventoryGRPC.Port()))
	if err != nil {
		return err
	}
...
func (a *App) runGRPCServer(ctx context.Context) error {
	logger.Info(ctx, fmt.Sprintf("🚀 gRPC InventoryService server listening on %d", config.AppConfig().InventoryGRPC.Port()))
